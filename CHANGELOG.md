...
本版本把订阅管理接进来：点账号卡片的订阅标签就能进 Stripe 账单页或直接开通套餐。
测活的模型选择改为可搜索的级联选择并支持推理档位，两个列表的筛选补上导入时间范围。

### 新增

#### 订阅管理

- 账号卡片的订阅类型标签可点，弹窗内先查订阅状态再分流：
  - 已订阅：拿到 Stripe 账单管理链接后用内置浏览器打开，改套餐 / 退订都在那边做
  - 未订阅：列出官方可开通的档位（名称、价格、计费周期、特性），并标出当前套餐
  - 查询失败：就地显示图标 + 标题 + 原始报错，附「关闭 / 重试」，不用关掉重开
- 每个档位一组按钮：左键生成支付链接并用内置浏览器打开，右键只把链接复制到剪贴板，
  想换个地方付款或先存下来都不用先打开浏览器
- 查询前按需为该账号续期 Token：门户的订阅接口认的是 cookie 里的 AccessToken，
  而 IDE 只续它自己在用的那个账号，不续期的话其余账号一律 401
- 内置浏览器新增「打开任意链接」入口，Stripe 页面与官网后台共用同一个私密会话分区

#### 测活支持推理档位

- 测活弹窗的模型列表改为可搜索的级联选择：一级是模型，二级是该模型自己的推理档位
  （Claude 系 low / medium / high / xhigh / max，GPT 系另有 none，没有这一层的模型不显示二级）
- 档位来自每个模型自己返回的 schema，只选主模型时自动补上该模型的默认档位，
  选中后选择器里显示「模型名 · 档位」；档位列顶部固定显示主模型名，
  避免几个模型档位重名时看不出选的是哪个
- 模型标题带上数量「模型（x个）」，x 为主模型数，不含二级档位
- API Key 测活同步支持，网关与测活共用同一份档位解析

#### 导入时间筛选

- 账号管理与 API Key 管理的筛选弹窗新增「导入时间」范围，精确到时分秒；
  只选日期不碰时间时自动补成起点 00:00:00 / 终点 23:59:59，否则两端都会落在 00:00:00

### 优化

- 工具栏那一排（筛选 / 分组 / 排序 / 刷新 / 批量操作）跟随设置里的控件尺寸：
  选大尺寸时用标准尺寸，此前写死 small，和上方的添加 / 导入 / 导出差了两档
- 筛选弹窗加宽到 540px：导入时间那行是「日期 + 时分秒」两段，460px 下会被挤到换行
- 测活成功后的提示去掉正确图标与「请求模型」那条描述，只留一行结果摘要
...
并把虚拟滚动从「定高」升级为「逐行可变高」，只给真有报错的那一行加高。

### 新增

#### 自定义分组

- 工具栏新增「分组」按钮，浮层内可新建、改名、删除分组，一排两个，长按可拖动排序；
  右键或右侧三点打开同一份菜单（改名 / 删除），删除有二次确认
- 点分组即按其筛选，支持多选，「未分组」也能作为一项参与筛选；
  分组按钮有独立角标，与筛选面板的条件互不干扰
- 批量操作新增「批量设置分组」：弹窗内同样能新建、改名、删除、拖动排序，选中后保存即归组；
  左下角另有「移出分组」，二次确认后把所选账号 / Key 变回未分组
- 账号卡片与 API Key 卡片在标签行显示分组名；名字过长时截断为省略号，
  剩余宽度全归它用，不换行也不会把同排的其他标签挤下去
- 删除分组只影响归属：其下的账号 / Key 变为未分组，本身不会被删除
- 分组定义随账号备份一起导出；恢复时按「同名复用、否则新建」建立映射再回填，
  换机恢复不会只剩一堆悬空 id

#### 三种展示形态

- 隐私打码右侧新增展示形态下拉，两个列表各记一份：
  - 卡片模式：现有观感
  - 卡片紧凑模式：收起额度明细 / 网关统计，用量块压成两行（柱状条与百分比同排，
    下面是总额与更新时间）
  - 列表模式：一排一个的横向长条，一屏能看几十条
- 列表模式按「卡片实际拿到的宽度」分四档重排（容器查询，不是窗口宽度 ——
  侧栏可折叠，两者并不相等）：宽屏一行放下四块，变窄后依次把标签、用量、
  操作区挪到下一行，各档之间只是重排，不会挤压变形或横向溢出

#### 批量设置备注（API Key）

- API Key 的批量操作补上「批量设置备注」，覆盖式改写所选 Key 的备注，留空即清空。
  走主进程的批量接口，几百个 Key 也只落一次盘

### 优化

- 卡片改为整卡点选：除身份区（开详情）、用量块（开积分变化）、统计格（开曲线）
  和右下角动作按钮外，点卡片任意位置都能勾选 / 取消勾选
- 虚拟滚动支持逐行可变高：行顶偏移预先算成前缀和、滚动时二分定位。
  此前是统一行高且只增不减，一旦滑过一张带报错的卡片，之后每张卡片都留着那条空白；
  现在只有「同一行里真有报错」的那一行才加高
- API Key 卡片不再为报错行恒定预留 27px，并整体收紧内边距与块间距（一张卡约省 30px）；
  底部动作区改为钉在卡片底边，富余高度落在它上方
- 窗口宽度变化后重新测量单元格高度（此前只允许变高，窄屏折行再拉回宽屏会一直卡在最高值）
- 分组面板、批量分组、批量备注、展示形态下拉与分组网格抽成公用组件，
  账号与 API Key 两侧共用一份；分组读写差异（渲染层同步写 / 主进程 IPC）收敛到一个门面

### 修复

- 修复分组面板里点改名弹窗的输入框会导致整个面板连弹窗一起消失：
  弹窗 teleport 到 body，被 popover 判定成「点了外面」而收起，现在弹窗在场时按住不放
- 修复长按拖动排序完全失效：Chromium 在 mousedown 那一刻就决定手势是否为拖拽，
  长按后再置 `draggable` 不会触发 dragstart，改为全程指针事件实现
- 修复带报错的卡片在列表模式下把操作区挤到下一行留出一块空白：
  网格自动摆放的游标不能回退，改为每块写死落位
- 修复筛选面板的重置会顺带清掉分组筛选
...
[1.0.24]: https://github.com/lucks-cloud/kiro-manager-lite/releases/tag/v1.0.24
